...
class futures_price_base(ZC_Base, Plot_Base):
...
class ZC01HeYueShouPanJia(futures_price_base, Computed):
    field_name = u"国内期货价格"
    col_name = u"ZC01合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_ZC_quote_df(month=1)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name
        return quote_ts.to_frame()


class ZC05HeYueShouPanJia(futures_price_base, Computed):
    field_name = u"国内期货价格"
    col_name = u"ZC05合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_ZC_quote_df(month=5)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name
        return quote_ts.to_frame()


class ZC09HeYueShouPanJia(futures_price_base, Computed):
    field_name = u"国内期货价格"
    col_name = u"ZC09合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_ZC_quote_df(month=9)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name  
        return quote_ts.to_frame()
