# -*- coding: utf-8 -*-
"""
Created on Fri Nov  2 21:17:53 2018
...
#from MyFutureClass.CmtDataBase.Commodities.L.L_Base import L_Base
#from MyFutureClass.CmtDataBase.Commodities.L import L_Device_SQLTable
#from MyFutureClass.Data.Trade_Date import Trade_Date
#
#from sqlalchemy.orm import sessionmaker
#from sqlalchemy import and_

file_data_path = "D:\Work\CommodityDataBase\Commodities\L/files/Device_Files/"
...
def liscence_to_specific_field(x):        
...
def field_to_device_list(ts, field=u"all"):
    tmp_ts = ts.dropna()
    if field == u"all":
...
class L_Device_Process(object):
    ...
    def local_hist_data_process():
        local_data_path = "D:\Work\CommodityDataBase\Commodities\L/local/"        
...
#                continue
    ...
    @staticmethod
    def L_Device_Update():
        dstfile =  "D:\Work\CommodityDataBase\Commodities\L/update/update_backup/" + datetime.now().strftime("%Y-%m-%d-%H_%M_%S")
        srcfile = "D:\Work\CommodityDataBase\Commodities\L/update/update_source/update_source.xlsx"
        os.makedirs(dstfile)
...
#        local_data_path = "D:\Work\CommodityDataBase\Commodities\L/update/update_source/"        
        hist_table = pd.read_excel(u"D:\Work\CommodityDataBase\Commodities\L/update/update_source/device_update.xlsx", encoding="gbk") 
        hist_table.iloc[:, [0, 1]] = hist_table.iloc[:, [0, 1]].ffill()
        liscence_list = []
        for i in range(len(hist_table)):
            tmp_company = hist_table.iloc[i][u"所属"]
    ...
    def liscence_df_to_field(liscence_df): 
        ...
        for col in liscence_df.columns:
            ...
            tmp_field_series = tmp_series.apply(lambda x: liscence_to_specific_field(x))
...
class L_Device_Factory(object):
    @staticmethod
    def get_device_cls(company, device):
...
class L_Device(object):
    ...
    @staticmethod
    def get_company_list_for_field(field=u"all"):
        total_field_df = L_Device.get_all_field_table()
        tmp_ts = total_field_df.apply(lambda x: field_to_device_list(x, field), axis=1)
        return tmp_ts
...
#            tmp_date = tmp_series.index[d].strftime("%Y-%m-%d") if isinstance(tmp_series.index[d], datetime) else tmp_series.index[d]
    ...
    def update_table_generate(self, tmp_series):
        ...
        tmp_df["product_field"] = tmp_field_series.apply(lambda x: specific_field_to_field(x)).values.tolist()
...
#        return tmp_series
    ...
    def get_product_specific_field(self):
        ...
        field_ts = product_ts.apply(lambda x: liscence_to_specific_field(x))
        return field_ts    

    def get_product_field(self):
        product_ts = self.get_product_specific_field()
        field_ts = product_ts.apply(lambda x: specific_field_to_field(x))
        return field_ts    
...
#    
#    date_list = Trade_Date.get_date_list("2018-09-01", "2018-10-31")
#    ts_list = []
#    for tmp_date in date_list:
#        tmp_date = datetime(tmp_date.year, tmp_date.month, tmp_date.day).strftime("%Y-%m-%d")
#        tmp_df = L_Device_Process.get_total_product_field_for_date(tmp_date, u"低压")
#        tmp_df["tmp"] = "_"
...
#    df.T.to_excel("HD.xlsx", encoding="gbk")
