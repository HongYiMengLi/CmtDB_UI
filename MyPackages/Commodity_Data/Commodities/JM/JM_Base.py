...
class JM_Base(CommonDataBaseClass):
    cmt_name = "JM"
    ...
    @staticmethod
    def get_JM_quote_df(month=None):
        tmp_df = QuoteData.Daily.get_cmt_quote("close", "JM.DCE", month=month)
        return tmp_df

    @staticmethod
    def get_JM_main_month_quote_df():
        main_month_list = [1, 5, 9]
        df_list = []
        for x in main_month_list:
            tmp_df = QuoteData.Daily.get_cmt_quote("close", "JM.DCE", month=x)
            df_list.append(tmp_df)
        total_df = pd.concat(df_list, axis=1, sort=False)
        return total_df
    
if __name__ == "__main__":
    aa = JM_Base.get_col_index_df()