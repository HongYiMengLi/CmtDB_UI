...
class futures_price_base(I_Base, Plot_Base):
...
class TieKuang01HeYueShouPanJia(futures_price_base, Computed):
    field_name = u"国内期货价格"
    col_name = u"I01合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_I_quote_df(month=1)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name
        return quote_ts.to_frame()


class TieKuang05HeYueShouPanJia(futures_price_base, Computed):
    field_name = u"国内期货价格"
    col_name = u"I05合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_I_quote_df(month=5)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name
        return quote_ts.to_frame()


class TieKuang09HeYueShouPanJia(futures_price_base, Computed):
    field_name = u"国内期货价格"
    col_name = u"I09合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_I_quote_df(month=9)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name  
        return quote_ts.to_frame()
