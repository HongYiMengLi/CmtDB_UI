...
class futures_price_base(HC_Base, Plot_Base):
...
class HC01HeYueShouPanJia(futures_price_base, Computed):
    field_name = u"国内期货价格"
    col_name = u"HC01合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_HC_quote_df(month=1)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name
        return quote_ts.to_frame()


class HC05HeYueShouPanJia(futures_price_base, Computed):
    field_name = u"国内期货价格"
    col_name = u"HC05合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_HC_quote_df(month=5)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name
        return quote_ts.to_frame()


class HC10HeYueShouPanJia(futures_price_base, Computed):
    field_name = u"国内期货价格"
    col_name = u"HC10合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_HC_quote_df(month=10)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name  
        return quote_ts.to_frame()
