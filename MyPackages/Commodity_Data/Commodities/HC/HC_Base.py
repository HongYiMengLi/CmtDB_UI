...
class HC_Base(CommonDataBaseClass):
    cmt_name = "HC"
    ...
    @staticmethod
    def get_HC_quote_df(month=None):
        tmp_df = QuoteData.Daily.get_cmt_quote("close", "HC.SHF", month=month)
        return tmp_df

    @staticmethod
    def get_HC_main_month_quote_df():
        main_month_list = [1, 5, 10]
        df_list = []
        for x in main_month_list:
            tmp_df = QuoteData.Daily.get_cmt_quote("close", "HC.SHF", month=x)
            df_list.append(tmp_df)
        total_df = pd.concat(df_list, axis=1, sort=False)
        return total_df
    
if __name__ == "__main__":
    aa = HC_Base.get_col_index_df()