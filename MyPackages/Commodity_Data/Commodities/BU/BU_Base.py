...
class BU_Base(CommonDataBaseClass):
    cmt_name = "BU"
    ...
    @staticmethod
    def get_BU_quote_df(month=None):
        tmp_df = QuoteData.Daily.get_cmt_quote("close", "BU.SHF", month=month)
        return tmp_df


    @staticmethod
    def get_BU_main_month_quote_df():
        main_month_list = [6, 9, 12]
        df_list = []
        for x in main_month_list:
            tmp_df = QuoteData.Daily.get_cmt_quote("close", "BU.SHF", month=x)
            df_list.append(tmp_df)
        total_df = pd.concat(df_list, axis=1, sort=False)
        return total_df
    
if __name__ == "__main__":
    aa = BU_Base.get_BU_quote_df()