...
class futures_price_base(BU_Base, Plot_Base):
...
class BUZhuLiHeYueShouPanJia(futures_price_base, WindData):
    field_name = u"期货价格"
    col_name = u"BU主力合约收盘价"
    wind_code = "S0182150"

        
""" 计算指标 """    
    
class BU06HeYueShouPanJia(futures_price_base, Computed):
    field_name = u"期货价格"
    col_name = u"BU06合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_BU_quote_df(month=6)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name
        return quote_ts.to_frame()


class BU09HeYueShouPanJia(futures_price_base, Computed):
    field_name = u"期货价格"
    col_name = u"BU09合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_BU_quote_df(month=9)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name
        return quote_ts.to_frame()


class BU12HeYueShouPanJia(futures_price_base, Computed):
    field_name = u"期货价格"
    col_name = u"BU12合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_BU_quote_df(month=12)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name  
        return quote_ts.to_frame()
