...
class A_Base(CommonDataBaseClass):
    cmt_name = "A"
    ...
    @staticmethod
    def get_A_quote_df(month=None):
        tmp_df = QuoteData.Daily.get_cmt_quote("close", "A.DCE", month=month)
        return tmp_df

    @staticmethod
    def get_A_main_month_quote_df():
        main_month_list = [1, 5, 9]
        df_list = []
        for x in main_month_list:
            tmp_df = QuoteData.Daily.get_cmt_quote("close", "A.DCE", month=x)
            df_list.append(tmp_df)
        total_df = pd.concat(df_list, axis=1, sort=False)
        return total_df
    
if __name__ == "__main__":
    aa = A_Base.get_col_index_df()