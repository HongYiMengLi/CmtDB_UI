...
class OI_Base(CommonDataBaseClass):
    cmt_name = "OI"
    ...
    @staticmethod
    def get_OI_quote_df(month=None):
        tmp_df = QuoteData.Daily.get_cmt_quote("close", "OI.CZC", month=month)
        return tmp_df

    @staticmethod
    def get_OI_main_month_quote_df():
        main_month_list = [1, 5, 9]
        df_list = []
        for x in main_month_list:
            tmp_df = QuoteData.Daily.get_cmt_quote("close", "OI.CZC", month=x)
            df_list.append(tmp_df)
        total_df = pd.concat(df_list, axis=1, sort=False)
        return total_df
    
if __name__ == "__main__":
    aa = OI_Base.get_col_index_df()