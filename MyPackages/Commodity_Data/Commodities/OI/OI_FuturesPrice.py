...
class futures_price_base(OI_Base, Plot_Base):
...
class OI01HeOIueShouPanJia(futures_price_base, Computed):
    field_name = u"国内期货价格"
    col_name = u"OI01合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_OI_quote_df(month=1)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name
        return quote_ts.to_frame()


class OI05HeOIueShouPanJia(futures_price_base, Computed):
    field_name = u"国内期货价格"
    col_name = u"OI05合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_OI_quote_df(month=5)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name
        return quote_ts.to_frame()


class OI09HeOIueShouPanJia(futures_price_base, Computed):
    field_name = u"国内期货价格"
    col_name = u"OI09合约收盘价"
    def get_ts_whole_progress(self):
        quote_df = self.get_OI_quote_df(month=9)
        quote_ts = month_quote_df_2_series(quote_df)
        quote_ts.name = self.col_name  
        return quote_ts.to_frame()
...
if __name__ == "__main__":
    df = OI12HeOIueShouPanJia().get_ts()
